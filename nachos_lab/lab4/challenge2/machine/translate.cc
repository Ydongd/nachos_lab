...
// from the virtual address
    ...
    if (tlb == NULL) {		// => page table => vpn is index into table
        ...
        else if (!currentThread->space->pageTable[vpn].valid) {
            DEBUG('a', "virtual page # %d too large for page table size %d!\n",
                  virtAddr, pageTableSize);
        ...
        for (entry = NULL, i = 0; i < TLBSize; i++){
    	    if (tlb[i].valid && (tlb[i].virtualPage == vpn) && tlb[i].tid == currentThread->getTID()) {
                entry = &tlb[i];			// FOUND!
                
                //update LRUtime
                for(int j=0; j<TLBSize; ++j){
                    if(tlb[j].valid && LRUtime[j] < LRUtime[i])
                        LRUtime[j]++;
                }
                LRUtime[i] = 0;
    ...
    for(int i=0; i<NumPhysPages; ++i){
        if(machine->memoryMap->Test(i) && pageLRUtime[i] < pageLRUtime[pageFrame])
            pageLRUtime[i]++;
    }
    pageLRUtime[pageFrame] = 0;
