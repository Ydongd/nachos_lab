...
int
OpenFile::oldReadAt(char *into, int numBytes, int position)
{
    ...
    time_t timep;
    time(&timep);
    hdr->setLastVisitTime(asctime(gmtime((&timep))));
...
// read in first and last sector, if they are to be partially modified
    if (!firstAligned)
        oldReadAt(buf, SectorSize, firstSector * SectorSize);
    if (!lastAligned && ((firstSector != lastSector) || firstAligned))
        oldReadAt(&buf[(lastSector - firstSector) * SectorSize],
...
// write modified sectors back
    for (i = firstSector; i <= lastSector; i++)	
     ...
     time(&timep);
     hdr->setLastVisitTime(asctime(gmtime((&timep))));
     hdr->setLastModifyTime(asctime(gmtime((&timep))));
