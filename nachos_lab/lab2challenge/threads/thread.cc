...
Thread::Thread(char* threadName)
{
    ...
    if(threadNum >= maxThreadNum){
        printf("WRONG : Thread limit exceeded!\n");
        flag = 0;
    }
...
Thread::Thread(char* threadName, int x)
{
    int flag = 1;
    if(threadNum > maxThreadNum){
        printf("WRONG : Thread limit exceeded!\n");
...
Thread::Finish ()
{
    ...
    printf("Thread %d finished! Time now: %d\n", currentThread->getTID(), stats->totalTicks);
    
    //recycling thread
    threadNum--;
    myThreads[this->getTID()] = NULL;
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
    printf("Yield: Thread %d to Thread %d\n", this->getTID(), nextThread->getTID());
