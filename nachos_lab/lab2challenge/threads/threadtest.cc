...
void mySliceTest(int which){
    while(currentThread->getTimeNeeded() > 0){
...
void ThreadTest5(){
    ...
    for(int i=0; i<2; ++i){
        tmp[i] = new Thread("1");
        tmp[i]->Fork(mySliceTest, 1);
        printf("Create Thread %d, its time is %d, its slice is %d\n",
               tmp[i]->getTID(), tmp[i]->getTimeNeeded(), tmp[i]->getTimeSlice());
    }
    currentThread->Yield();
}
